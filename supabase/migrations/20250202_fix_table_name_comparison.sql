-- ============================================================================
-- Fix: Table Name Comparison in fn_scd2_patch_base
-- Purpose: Fix "Table tcm_user_tally_card_entries not supported" error
-- Root Cause: p_table::text may return 'tcm_user_tally_card_entries' 
--             instead of 'public.tcm_user_tally_card_entries' when schema
--             is in search_path. The comparison needs to handle both cases.
-- Solution: Check both with and without schema prefix
-- ============================================================================

-- Fix the table name comparison to handle schema prefix variations
...
BEGIN
  -- Get table name in a way that handles schema prefix variations
  v_table_text := format('%s', p_table);
  -- Extract just the table name (without schema) for comparison
  v_table_name_only := substring(v_table_text from '\.([^.]+)$');
  IF v_table_name_only IS NULL THEN
    v_table_name_only := v_table_text;
  END IF;
  
  RAISE NOTICE '[SCD2-BASE] Starting for table: % (text: %, name_only: %)', p_table, v_table_text, v_table_name_only;
  ...
  BEGIN
    -- FIX: Check both with and without schema prefix
    IF v_table_text = 'public.tcm_user_tally_card_entries' OR v_table_name_only = 'tcm_user_tally_card_entries' THEN
      ...
      SELECT
        ...
        now()  -- CRITICAL: Always use current timestamp, NEVER accept updated_at from JSONB
    ...
    ELSIF v_table_text = 'public.tcm_tally_cards' OR v_table_name_only = 'tcm_tally_cards' THEN
      ...
      SELECT
        ...
        now()  -- CRITICAL: Always use current timestamp, NEVER accept snapshot_at from JSONB
      RETURNING id INTO v_new_id;
      
    ELSE
      RAISE EXCEPTION 'Table % (text: %, name_only: %) not supported in fn_scd2_patch_base', p_table, v_table_text, v_table_name_only;
    END IF;
...
-- Summary
-- ============================================================================
-- This fix makes the table name comparison robust by:
-- 1. Using format('%s', regclass) to get the full table name
-- 2. Extracting just the table name (without schema) for comparison
-- 3. Checking both 'public.tcm_user_tally_card_entries' AND 'tcm_user_tally_card_entries'
-- This handles cases where regclass::text might omit the schema prefix
