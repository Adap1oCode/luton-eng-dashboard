// -----------------------------------------------------------------------------
// FILE: src/app/(main)/forms/{{routeSegment}}/new/{{routeSegment}}-new-client.tsx
// TYPE: Client Component
// PURPOSE: Create new {{resourceTitle}} form
// -----------------------------------------------------------------------------
...
export function {{pascalCase(routeSegment)}}NewClient() {
  ...
  const form = useForm<{{pascalCase(resourceKey)}}FormData>({
    ...
    defaultValues: {
      {{#each formFields}}
      {{field}}: {{#if (eq type "string")}}""{{else if (eq type "number")}}0{{else if (eq type "boolean")}}false{{else}}""{{/if}},
      {{/each}}
    },
  });
  ...
  const createMutation = useMutation({
    mutationFn: async (data: {{pascalCase(resourceKey)}}FormData) => {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.message || "Failed to create {{resourceTitle}}");
      }
    ...
    onSuccess: () => {
      ...
      toast.success("{{resourceTitle}} created successfully");
      router.push("/forms/{{routeSegment}}");
    },
    onError: (error: Error) => {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Create New {{resourceTitle}}</CardTitle>
      </CardHeader>
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="flex justify-end space-x-2">
              ...
              <Button type="submit" disabled={isSubmitting}>
                {isSubmitting ? "Creating..." : "Create {{resourceTitle}}"}
              </Button>
            </div>
          </form>
        </Form>
