...
**Stock Adjustments View — Full Audit**

You are a senior Next.js/TypeScript reviewer. Your task is to perform a discovery-driven audit of the Stock Adjustments – View screen end-to-end and identify the smallest set of changes that cut unnecessary complexity and render/data overhead — without changing behavior.
...
Map the runtime path when the user opens the /forms/stock-adjustments screen:
...
```
Perform a full discovery audit of the [SCREEN NAME] – View screen.
Use the same process we used for Stock Adjustments View (runtime trace → dependency map → transformation audit → payload check → hotspots → 80/20 plan).
...
- A 7-section Markdown report titled e.g. `VIEW_AUDIT.md`
