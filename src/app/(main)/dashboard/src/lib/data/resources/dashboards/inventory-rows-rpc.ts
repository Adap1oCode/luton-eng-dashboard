...
export async function getInventoryRows(
  ...
  try {
    ...
    // Convert filters to the correct RPC format
    let rpcFilter = {};
    if (filters && Object.keys(filters).length > 0) {
      // Convert dashboard filter format to RPC format
      if (filters.column && filters.equals !== undefined) {
        rpcFilter = {
          column: filters.column,
          op: "=",
          value: filters.equals
        };
      } else if (filters.column && filters.isNotNull) {
        rpcFilter = {
          column: filters.column,
          op: "IS NOT NULL"
        };
    ...
    console.log("🔍 [RPC] Converted filter:", rpcFilter);
    
    // Call the existing RPC function with correct parameter names
