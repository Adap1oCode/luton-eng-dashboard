...
import { permissionCreateConfig } from "./form.config";

export default async function NewPermissionPage() {
  const cfg = ensureSections(permissionCreateConfig);
  ...
  return (
    <FormShell
      ...
      actions={{
        secondaryLeft: (
          <a href="/forms/permissions" className="inline-flex items-center rounded-md border px-4 py-2 text-sm">
            Cancel
          </a>
        ),
        primary: (
          <PermissionGate any={["screen:permissions:create"]}>
            <button
              form={formId}
              type="submit"
