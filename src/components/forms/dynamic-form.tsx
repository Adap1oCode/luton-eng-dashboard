...
function colStartClass(n: number) {
  switch (n) {
    ...
    case 4: return "lg:col-start-4";
    default: return "lg:col-start-1";
  }
}
...
function SectionBody({
  ...
  const prepared = (fill === "column"
    ? autoPlaceColumnFirst(section.fields, columns)
    : autoPlaceRowFirst(section.fields, columns)
  ).map((f) => ({
    ...f,
      ...
      {prepared.map((f) => {
        ...
        return (
          <div key={f.name} className={["space-y-0", startCls, spanCls].join(" ")}>
            <DynamicField
              field={f}
              options={f.optionsKey ? options[f.optionsKey] : undefined}
            />
          </div>
        );
      })}
  ...
  function normalizeDefaults(vals: Record<string, any>) {
    ...
    for (const f of allFields) {
      ...
      if (v === null || v === undefined) {
        ...
        else if (f.kind === "number") out[f.name] = undefined; // show empty number input
