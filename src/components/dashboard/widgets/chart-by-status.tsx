...
export default function ChartByStatus({ data, accessor = "status", onFilterChange }: Props) {
      ...
      <CardContent className="px-2 pt-4 sm:px-6 sm:pt-6">
        <ResponsiveContainer width="100%" height={260}>
          <BarChart data={chartData} layout="vertical">
            ...
            <XAxis type="number" tick={{ fontSize: 12, fill: "var(--foreground)", fontWeight: 500 }} />
            <YAxis
              type="category"
              dataKey="label"
              width={180}
              ...
              content={({ payload }) =>
                payload && payload.length ? (
                  <div className="rounded-md border bg-white px-3 py-2 text-xs shadow-sm">
                    <div className="text-foreground font-medium">{payload[0].payload.label}</div>
