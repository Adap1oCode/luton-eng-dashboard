...
export async function getRequisitionMetrics(range: "3m" | "6m" | "12m") {
  ...
  return {
    ...
    missingOrderDate: metric(current.filter((r) => !r.order_date).length, previous.filter((r) => !r.order_date).length),
    missingDueDate: metric(current.filter((r) => !r.due_date).length, previous.filter((r) => !r.due_date).length),
  };
}
