...
function mapError(resourceKey: string, err: unknown) {
  ...
  return json(
    {
      error: {
        message: (err as any)?.message ?? (typeof err === "string" ? err : "Internal server error"),
      },
...
/** PATCH (update) */
export async function updateHandler(req: Request, resourceKey: string, id: string) {
  const invalid = validateParams(resourceKey, id);
  if (invalid) return invalid;
  ...
  try {
    ...
    const patch =
      typeof (entry.config as any).fromInput === "function" ? (entry.config as any).fromInput(payload) : payload;
...
/** DELETE (soft or hard) */
export async function deleteHandler(_req: Request, resourceKey: string, id: string) {
  const invalid = validateParams(resourceKey, id);
  if (invalid) return invalid;
